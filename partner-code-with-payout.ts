  ...
  try {
    ...
    if (action === "activate") {
      ...
      const currentBalance = safeNumber(
        ...
        await getCurrentBalance(supabase, user.id),
      );

      const totalDeposits = safeNumber(payload.totalDeposits, 0);
      const totalWithdrawals = safeNumber(payload.totalWithdrawals, 0);
