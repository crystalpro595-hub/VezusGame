...
async function getPartnerPayload(supabase: any) {
  ...
  const payoutTotalsByPartner: Record<string, any> = {};
  for (const payout of payoutRequests) {
    const partnerId = String(payout.partner_user_id || "");
    if (!partnerId) continue;
    if (!payoutTotalsByPartner[partnerId]) {
      payoutTotalsByPartner[partnerId] = { pending: 0, approved: 0, paid: 0, rejected: 0, total: 0 };
    }
    const amount = asMoneyNumber(payout.amount);
    const status = String(payout.status || "pending");
    payoutTotalsByPartner[partnerId].total += amount;
    if (status === "pending") payoutTotalsByPartner[partnerId].pending += amount;
    else if (status === "approved") payoutTotalsByPartner[partnerId].approved += amount;
    else if (status === "paid") payoutTotalsByPartner[partnerId].paid += amount;
    else if (status === "rejected") payoutTotalsByPartner[partnerId].rejected += amount;
  }
  ...
  const partners = partnerUsers
    ...
    .map((user: any) => ({
      ...
      payout_totals: payoutTotalsByPartner[String(user.id)] || { pending: 0, approved: 0, paid: 0, rejected: 0, total: 0 },
      paid_total: asMoneyNumber((payoutTotalsByPartner[String(user.id)] || {}).paid) + asMoneyNumber((earningTotalsByPartner[String(user.id)] || {}).paid),
  ...
  const earningsTotal = enrichedEarnings
    .filter((item: any) => item.status !== "cancelled")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.commission_amount), 0);

  const earningsPendingByStatus = enrichedEarnings
    .filter((item: any) => item.status === "pending")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.commission_amount), 0);

  const earningsAvailableByStatus = enrichedEarnings
    .filter((item: any) => item.status === "available")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.commission_amount), 0);

  const earningsPaidByStatus = enrichedEarnings
    .filter((item: any) => item.status === "paid")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.commission_amount), 0);

  const pendingBalanceTotal = partners.reduce(
    (sum: number, partner: any) => sum + asMoneyNumber(partner.partner_pending_balance),
    0,
  );

  const availableBalanceTotal = partners.reduce(
    (sum: number, partner: any) => sum + asMoneyNumber(partner.partner_balance),
    0,
  );

  const payoutPending = enrichedPayoutRequests
    .filter((item: any) => item.status === "pending")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.amount), 0);

  const payoutApproved = enrichedPayoutRequests
    .filter((item: any) => item.status === "approved")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.amount), 0);

  const payoutPaid = enrichedPayoutRequests
    .filter((item: any) => item.status === "paid")
    .reduce((sum: number, item: any) => sum + asMoneyNumber(item.amount), 0);

  const payoutRejected = enrichedPayoutRequests
  ...
  const overview = {
    ...
    earnings_pending: earningsPendingByStatus,
    earnings_available: earningsAvailableByStatus,
    earnings_paid: earningsPaidByStatus,

    pending_balance_total: pendingBalanceTotal,
    ...
    payout_total: payoutPending + payoutApproved + payoutPaid + payoutRejected,

    paid_total: payoutPaid + earningsPaidByStatus,
  };
...
async function getAdminPayload(supabase: any) {
  ...
  return {
    ...
    stats: {
      ...
      partner_pending_amount: partnerPayload.partner_overview.pending_balance_total,

      partner_available_amount: partnerPayload.partner_overview.available_balance_total,

      partner_paid_amount: partnerPayload.partner_overview.paid_total,
