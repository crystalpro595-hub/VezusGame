-- Vezus Cups: подключение к существующим таблицам GameSetting + GameRound
-- ВАЖНО: этот файл НЕ создаёт новые таблицы.
-- Он использует уже существующие public."GameSetting", public."GameRound", public."User", public."Balance".

-- Если ты уже запускал прошлый вариант, где была создана отдельная таблица CupsRound,
-- можешь удалить её отдельным файлом 00_optional_drop_wrong_cupsround.sql из пакета.

-- 1) Добавляем строку настройки Cups в существующую GameSetting, если её ещё нет.
-- Таблицу GameSetting не создаём и не меняем.
...
-- 2) Внутренние функции: аккуратно пишут финальный раунд в существующую GameRound.
-- Они проверяют, какие колонки реально есть в GameRound, и вставляют только их.
-- Если в твоей GameRound какая-то колонка имеет другой enum/check, функция повторит insert
-- более безопасным набором колонок.
...
-- 3) Получение состояния игры одним быстрым RPC.
create or replace function public.cups_get_state(p_telegram_id bigint)
returns jsonb
language plpgsql
security definer
set search_path = public
...
-- 4) Один атомарный раунд Cups:
-- проверяет баланс, берёт шанс из GameSetting, считает итог, обновляет Balance,
-- записывает финальный раунд в существующую GameRound.
...
begin
  ...
  if not found then
    ...
    values (v_user_id, p_telegram_id, 0, 'RUB', now())
  ...
  if v_is_win then
    ...
    -- При проигрыше шарик всегда показываем под другим стаканчиком.
